...
def highest_Vol_PE(num, step, nearest, url):
           # print("-------------------->>>>>>>>>>>>>>> Find HIGHEST VOLUME AT PE <<<<<<<<<<<<<----------------------------")
            ...
            for item in data['records']['data']:
                if item["expiryDate"] == currExpiryDate:
                    if item["strikePrice"] == strike and item["strikePrice"] < start_strike + (step * num * 2):
                        if item["PE"]["totalTradedVolume"] > max_vol:
...
def highest_Volume_Data_PE(num, step, nearest, url):
    # print("-------------------->>>>>>>>>>>>>>> Find HIGHEST VOLUME AT CE <<<<<<<<<<<<<----------------------------")
    strike = nearest - (step * num)
    start_strike = nearest - (step * num)
    response_text = get_data(url)
    ...
    for item in data['records']['data']:
        if item["expiryDate"] == currExpiryDate:
            if item["strikePrice"] == strike and item["strikePrice"] < start_strike + (step * num * 2):
                if item["PE"]["totalTradedVolume"] > max_vol_data:
                    max_vol_data = item["PE"]["totalTradedVolume"]
                    max_vol_strike = item["strikePrice"]
                strike = strike + step
    return max_vol_data
def print_HighestVolume(num,step,nearest,url):
    print("--------------->>>>>>>>>>> Printing Highest Vol Data <<<<<<<<<-------------------------")
    strike = nearest - (step*num)
    start_strike = nearest - (step*num)
    response_text = get_data(url)
    data = json.loads(response_text)
    ...
    for item in data['records']['data']:
        if item["expiryDate"] == currExpiryDate:
            if item["strikePrice"] == strike and item["strikePrice"] < start_strike+(step*num*2):
                ...
                print((data["records"]["expiryDates"][0] +",CE" + "," +
                       str(item["CE"]["changeinOpenInterest"]) + "," +str(item["CE"]["openInterest"]) +","+str(item["CE"]["totalTradedVolume"])+","+" "+ str(item["strikePrice"]) +","+ " PE,"  +
                       str(item["PE"]["totalTradedVolume"]) + "," +str(item["PE"]["openInterest"]) +"," + str(item["PE"]["changeinOpenInterest"])))
                #     x=open(fName,'a')
                #     print((data["records"]["expiryDates"][0] + " " + ",CE " + ","+
                #         str(item["CE"]["changeinOpenInterest"]) + "," +str(item["CE"]["openInterest"]) +","+str(item["CE"]["totalTradedVolume"])+","+" "+ str(item["strikePrice"]) +","+ " PE,"  +
                #             str(item["PE"]["totalTradedVolume"]) + "," +str(item["PE"]["openInterest"]) +"," + str(item["PE"]["changeinOpenInterest"])+","),file=x)
...
#Finding Highest Chng In OI in PUT Option in Nifty
nf_highestOiChng_PE = highest_ChngInOI_PE(10,50,nf_nearest,url_nf)

#Finding Highest OI Chang Data in PUT in CALL
nf_highestOiChng_PE_Data = highest_ChngInOI_PE_Data(10,50,nf_nearest,url_nf)

#Finding Highest Vol in call option in Nifty
nf_highestVol_CE = highest_Vol_CE(10,50,nf_nearest,url_nf)

#Finding highest volumen data in call side
nf_highestVolData_CE = highest_Volume_Data_CE(10,50,nf_nearest,url_nf)

#Finding Highest Vol in put option in Nifty
nf_highestVol_PE = highest_Vol_PE(10,50,nf_nearest,url_nf)

#Finding Highest Vol in put option in Nifty
nf_highestVolData_PE = highest_Volume_Data_PE(10,50,nf_nearest,url_nf)

#t_notify_data='Major Resistance in Nifty:' + str(nf_highestVol_CE)+'\n'+'\n'+'Major Support in Nifty:' + str(nf_highestVol_PE)+'\n'+'\n'+'Major Resistance in Bank Nifty:' + str(bnf_highestoi_CE)+'\n'+'\n'+'Major Support in Bank Nifty:' + str(bnf_highestoi_PE)+'\n'+'\n'+'Highest Chng In OI CE |STRIKE PRICE| AND |ChngInOIData| :-> '+ str(nf_highestOiChng_CE) +' : '+ 'CE |'+str(nf_highestOiChng_CE_Data)+'|'+'\n'+'\n'+'Highest Chng In OI PE |STRIKE PRICE| AND |ChngInOIData| :-> '+ str(nf_highestOiChng_PE) +' : '+ 'PE |'+str(nf_highestOiChng_PE_Data)+'|'+'\n'+'\n'+'Highest VOLUME in CE |STRIKE PRICE| AND |RESISTANCE|:-> ' + str(nf_highestVol_CE)+ ' : '+ 'CE | '+str(nf_highestVolData_CE)+'|'+'\n'+'\n'+'Highest VOLUME in PE |STRIKE PRICE| AND |SUPPORT|:-> ' + str(nf_highestVol_PE)+ ' : '+ 'PE | '+str(nf_highestVolData_PE)+'|'+'\n'+'\n'+'Highest OI CE |STRIKE PRICE| AND |OI| :-> '+ str(nf_highestoi_CE) +' : '+ 'CE | '+str(nf_highestoi_ce_data)+' |'+'\n'+'\n'+'Highest OI PE |STRIKE PRICE| AND |OI| :-> '+ str(nf_highestoi_PE) +' : '+ 'PE | '+str(nf_highestoi_pe_data)+' |'

#t_url="https://api.telegram.org/bot5771720913:AAH0A70f0BPtPjrOCTrhAb9LR7IGFBVt-oM/sendMessage?chat_id=-703180529&text="+t_notify_data
#requests.post(t_url)
...
print(strCyan(str("Resistance in Nifty:")) + str(nf_highestVol_CE)+"\n"+"\n")
print(strCyan(str("Support in Nifty:")) + str(nf_highestVol_PE)+"\n"+"\n")
...
print(strGreen(str("Highest VOLUME in PE |STRIKE PRICE| AND |SUPPORT|: -> "))+"  "+str(nf_highestVol_PE)+ " : "+ "PE | "+str(nf_highestVolData_PE)+" |"+"\n"+"\n")
...
#url="https://api.telegram.org/bot5771720913:AAH0A70f0BPtPjrOCTrhAb9LR7IGFBVt-oM/sendMessage?chat_id=-1001714168970&text="+str('Major Resistance in Nifty:') + str(nf_highestVol_CE)+'\n'+str('Major Support in Nifty:') + str(nf_hihestVol_PE)+'\n'+('Major Resistance in Bank Nifty:") + str(bnf_highestoi_CE)+"\n"+str("Major Support in Bank Nifty:") + str(bnf_highestoi_PE)+"\n"+str("Highest Chng In OI CE |STRIKE PRICE| AND |ChngInOIData| :-> ")+ str(nf_highestOiChng_CE) +" : "+ "CE |"+str(nf_highestOiChng_CE_Data)+"|"+"\n"+str("Highest Chng In OI PE |STRIKE PRICE| AND |ChngInOIData| :-> ")+ str(nf_highestOiChng_PE) +" : "+ "PE |"+str(nf_highestOiChng_PE_Data)+"|"+"\n"+str("Highest VOLUME in CE |STRIKE PRICE| AND |RESISTANCE|:-> ") + str(nf_highestVol_CE)+ " : "+ "CE | "+str(nf_highestVolData_CE)+"|"+"\n"+str("Highest VOLUME in CE |STRIKE PRICE| AND |RESISTANCE|:-> ") + str(nf_highestVol_CE)+ " : "+ "CE | "+str(nf_highestVolData_CE)+"|"+str("Highest OI CE |STRIKE PRICE| AND |OI| :-> ")+ str(nf_highestoi_CE) +" : "+ "CE | "+str(nf_highestoi_ce_data)+" |"+"\n"+str("Highest OI PE |STRIKE PRICE| AND |OI| :-> ")+ str(nf_highestoi_PE) +" : "+ "PE | "+str(nf_highestoi_pe_data)+" |"
#url="https://api.telegram.org/bot5771720913:AAH0A70f0BPtPjrOCTrhAb9LR7IGFBVt-oM/sendMessage?chat_id=-703180529&text='Hello World'"
#requests.post(url)

#url="https://api.telegram.org/bot5771720913:AAH0A70f0BPtPjrOCTrhAb9LR7IGFBVt-oM/sendMessage?chat_id=-1001714168970&text="+str("Major Resistance in Nifty:") + str(nf_highestVol_CE)+"\n"+str("Major Support in Nifty:") + str(nf_hihestVol_PE)+"\n"+("Major Resistance in Bank Nifty:") + str(bnf_highestoi_CE)+"\n"+str("Major Support in Bank Nifty:") + str(bnf_highestoi_PE)+"\n"+str("Highest Chng In OI CE |STRIKE PRICE| AND |ChngInOIData| :-> ")+ str(nf_highestOiChng_CE) +" : "+ "CE |"+str(nf_highestOiChng_CE_Data)+"|"+"\n"+str("Highest Chng In OI PE |STRIKE PRICE| AND |ChngInOIData| :-> ")+ str(nf_highestOiChng_PE) +" : "+ "PE |"+str(nf_highestOiChng_PE_Data)+"|"+"\n"+str("Highest VOLUME in CE |STRIKE PRICE| AND |RESISTANCE|:-> ") + str(nf_highestVol_CE)+ " : "+ "CE | "+str(nf_highestVolData_CE)+"|"+"\n"
#+str("Highest VOLUME in CE |STRIKE PRICE| AND |RESISTANCE|:-> ") + str(nf_highestVol_CE)+ " : "+ "CE | "+str(nf_highestVolData_CE)+"|"+str("Highest OI CE |STRIKE PRICE| AND |OI| :-> ")+ str(nf_highestoi_CE) +" : "+ "CE | "+str(nf_highestoi_ce_data)+" |"+"\n"+str("Highest OI PE |STRIKE PRICE| AND |OI| :-> ")+ str(nf_highestoi_PE) +" : "+ "PE | "+str(nf_highestoi_pe_data)+" |"
#url="https://api.telegram.org/bot5771720913:AAH0A70f0BPtPjrOCTrhAb9LR7IGFBVt-oM/sendMessage?chat_id=-703180529&text='Hello World'"#+str("Major Resistance in Nifty:") + str(nf_highestVol_CE)+"\n"+str("Major Support in Nifty:") + str(nf_hihestVol_PE)+"\n"+("Major Resistance in Bank Nifty:") + str(bnf_highestoi_CE)+"\n"+str("Major Support in Bank Nifty:") + str(bnf_highestoi_PE)+"\n"+str("Highest Chng In OI CE |STRIKE PRICE| AND |ChngInOIData| :-> ")+ str(nf_highestOiChng_CE) +" : "+ "CE |"+str(nf_highestOiChng_CE_Data)+"|"+"\n"+str("Highest Chng In OI PE |STRIKE PRICE| AND |ChngInOIData| :-> ")+ str(nf_highestOiChng_PE) +" : "+ "PE |"+str(nf_highestOiChng_PE_Data)+"|"+"\n"+str("Highest VOLUME in CE |STRIKE PRICE| AND |RESISTANCE|:-> ") + str(nf_highestVol_CE)+ " : "+ "CE | "+str(nf_highestVolData_CE)+"|"+"\n"+str("Highest VOLUME in CE |STRIKE PRICE| AND |RESISTANCE|:-> ") + str(nf_highestVol_CE)+ " : "+ "CE | "+str(nf_highestVolData_CE)+"|"+str("Highest OI CE |STRIKE PRICE| AND |OI| :-> ")+ str(nf_highestoi_CE) +" : "+ "CE | "+str(nf_highestoi_ce_data)+" |"+"\n"+str("Highest OI PE |STRIKE PRICE| AND |OI| :-> ")+ str(nf_highestoi_PE) +" : "+ "PE | "+str(nf_highestoi_pe_data)+" |"
exit()
